...
void slingshotControl(void * param) {
  while (true) {
  if (stateOP == 0) {
...
void opcontrol() {
  ...
  unwind.set_brake_mode(MOTOR_BRAKE_HOLD);
  ...
  while (true) {
    ...
    // . . .
      ...
      if (master.get_digital_new_press(E_CONTROLLER_DIGITAL_R1)) {
        ...
        intake = intakeBool*127;
      }

      if (master.get_digital(E_CONTROLLER_DIGITAL_B)) {
        if (master.get_digital(E_CONTROLLER_DIGITAL_RIGHT)) {
          endgames.set_value(true);
